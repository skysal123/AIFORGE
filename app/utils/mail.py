...
"""Mail helper using SendGrid Web API v3."""
import logging
import os
from threading import Thread
...
def _send_async(app, message):
    """Send message via SendGrid API in a background thread."""
    with app.app_context():
        try:
            ...
            log.info(
                "Enquiry email sent via SendGrid (status %s) to %s",
                response.status_code,
                message.to[0].email if message.to else "recipient",
            )
        except Exception:
            log.exception("Failed to send enquiry email via SendGrid API")


def send_enquiry_notification(enquiry):
    """Email the configured recipient about a new enquiry.

    Returns ``True`` if dispatch started, ``False`` if missing settings.
    """
    recipient = current_app.config.get("CONTACT_EMAIL")
    sender = current_app.config.get("MAIL_DEFAULT_SENDER") or "aiforgetechno@gmail.com"
    api_key = current_app.config.get("MAIL_PASSWORD")

    missing = [name for name, val in (
        ("CONTACT_EMAIL", recipient),
        ("MAIL_PASSWORD (SendGrid API Key)", api_key),
    ) if not val]

    if missing:
        log.warning(
            "Mail API not fully configured — missing %s. "
            "Enquiry from %s <%s> saved to DB but NOT emailed.",
            ", ".join(missing), enquiry.name, enquiry.email,
        )
        return False
